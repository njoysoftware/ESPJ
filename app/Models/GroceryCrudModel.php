...
class GroceryCrudModel extends Model
{
	...
	{
		$this->builder->orHaving($key, $value, $escape);
	}
	...
	function or_like($field, $match = '', $side = 'both')
	{
		...
		$this->builder->orLike($field, $match, $side)->get();
	}
	...
	function get_relation_array($field_name, $related_table, $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
	{
		...
		if ($search_like !== null)
			$this->builder->having("$field_name_hash LIKE '%" . $this->db->escapeLikeString($search_like) . "%'");
	...
	function db_file_delete($field_name, $filename)
	{
		...
		if ($builder->update(array($field_name => ''), array($field_name => $filename))) {
			return true;
		} else {
			return false;
		}
