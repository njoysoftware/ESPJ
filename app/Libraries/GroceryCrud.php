            ...
            if (is_array($state_info->search)) {
                foreach ($state_info->search as $search_field => $search_text) {
                    ...
                    if (isset($temp_relation[$search_field])) {
                        ...
                        $this->basic_model->like($search_field, $search_text);
                    }
                }
                ...
                if (isset($temp_relation[$state_info->search->field])) {
                    ...
                    if (is_array($temp_relation[$state_info->search->field])) {
                        $temp_where_query_array2 = [];
                        foreach ($temp_relation[$state_info->search->field] as $search_field) {
                            $temp_where_query_array2[] = $search_field . ' LIKE \'%' . $state_info->search->text . '%\'';
                            $this->where('(' . implode(' OR ', $temp_where_query_array2) . ')', null);
                            // $this->basic_model->or_like($search_field, $state_info->search->text);
                        }
                    } else {
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
    ...
    protected function _export_to_excel($data)
    {
        ...
        if (session()->get('role') <> 'admin') {
            ...
            $string_to_export .= "Rekap " . $data->subject . "\n";
            $string_to_export .= "Panwaslu " . $kecamatan . "\n";
            $string_to_export .= "Badan Pengawas Pemilihan Umum Kabupaten Lamongan";
            $string_to_export .= "\n";
            $string_to_export .= "Tanggal Cetak: " . date('d-m-Y') . "\n";
            $string_to_export .= "\n";
        } else {
            if ($data->subject === 'Bagi RAB') {
                $string_to_export = "";
            } else {
                $string_to_export = "";
                $string_to_export .= "Rekap " . $data->subject . "\n";
                $string_to_export .= "Panwaslu Se-Kabupaten Lamongan" . "\n";
                $string_to_export .= "Badan Pengawas Pemilihan Umum Kabupaten Lamongan";
                $string_to_export .= "\n";
                $string_to_export .= "Tanggal Cetak: " . date('d-m-Y') . "\n";
                $string_to_export .= "\n";
    ...
    protected function _print_webpage($data)
    {
        ...
        if (session()->get('role') <> 'admin') {
            ...
            $string_to_print .= "<div class='print-header' style='text-align: center; text-transform: uppercase; text-margin-bottom: 2px;'>
        <h2>Rekap " . $data->subject . "</h2>
        <h2> Panwaslu " . $kecamatan . "</h2>
        <h3>Badan Pengawas Pemilihan Umum Kabupaten Lamongan</h3>
        </div>";
            $string_to_print .= "<div class='print-table'><p>Tanggal Cetak: " . date('d-m-Y') . "</p></div>";
        } else {
            $string_to_print .= "<div class='print-header' style='text-align: center; text-transform: uppercase; text-margin-bottom: 2px;'>
        <h2>Rekap " . $data->subject . "</h2>
        <h2> Panwaslu Se-Kabupaten Lamongan</h2>
        <h3>Badan Pengawas Pemilihan Umum Kabupaten Lamongan</h3>
        </div>";
            $string_to_print .= "<div class='print-table'><p>Tanggal Cetak: " . date('d-m-Y') . "</p></div>";
        }
        ...
        //Tambahan 2
        if (isset($data->list[0]->id_kuitansi) && !isset($data->list[0]->id_pembukuan)) {
            $bruto = 0;
            $netto = 0;
            $ppn = 0;
            $pph = 0;
            foreach ($data->list as $cari_row => $data_row) {
                $bruto += $data_row->bruto;
                $ppn += $data_row->ppn;
                $pph += $data_row->pph;
                $netto += $data_row->netto;
            }
        }
        //Tambahan 2
        if (isset($data->list[0]->id_pembukuan)) {
            $debit = 0;
            $kredit = 0;

            foreach ($data->list as $cari_row => $data_row) {
                $debit += $data_row->debet;
        ...
        $string_to_print .= '<table width="100%" cellpadding="2px" cellspacing="2px" ><tr>';
        ...
        //BKU
        if (isset($data->list[0]->id_pembukuan)) {
            $string_to_print .= "<tr><td colspan=3; style='text-align: center'; ><b>Total</b> </td><td>" . number_format($debit, 0, ',', '.') . "</td>";
            $string_to_print .= "<td>" . number_format($kredit, 0, ',', '.') . "</td></tr>";
        }

        $string_to_print .= "</table></div>";
        if (isset($data->list[0]->id_kuitansi) && !isset($data->list[0]->id_pembukuan)) {
            $string_to_print .= "<div style='margin-top:20px'>";
            $string_to_print .= '<table width="10%" cellpadding="2px" cellspacing="20px" ><tr>';

            $string_to_print .= "<b>Total Bruto: Rp. " . number_format($bruto, 0, ',', '.') . "</b></div>";
            $string_to_print .= "<p>Total PPN:Rp. " . number_format($ppn, 0, ',', '.') . "</p></div>";
            $string_to_print .= "<p>Total PPH:Rp. " . number_format($pph, 0, ',', '.') . "</p></div>";
            $string_to_print .= "<b>Total Netto:Rp. " . number_format($netto, 0, ',', '.') . "</b></div>";
        ...
        //Tambahan 3
        if (session()->get('role') <> 'admin') {
            $string_to_print .= "<div class='print-footer' style='text-align: right; margin-top: 100px;'>
