...
class Connection extends BaseConnection
{
    ...
    public function connect(bool $persistent = false)
    {
        ...
        try {
            ...
            if ($databasePath !== ':memory:' && ! str_contains($databasePath, DIRECTORY_SEPARATOR)) {
                $databasePath = WRITEPATH . $databasePath;
            }

            $sqlite = (! isset($this->password) || $this->password !== '')
                ? new SQLite3($databasePath)
                : new SQLite3($databasePath, SQLITE3_OPEN_READWRITE | SQLITE3_OPEN_CREATE, $this->password);
